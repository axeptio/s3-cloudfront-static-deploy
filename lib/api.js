...
function publishFile(clients, {
  bucket, distributionId, filename, content, contentType, cloudfrontFilename
}, onEvent) {
  return new Promise((resolve, reject) => {
    (async () => {
      try {
...
async function publishFiles(clients, {
  bucket, distributionId, filenames, contents, contentType
}, onEvent) {
  try {
    const actions = [
    ...
    success(onEvent, 'publishFiles', 'the files were successfully created or modified on s3 and invalidated on cloudfront');
    return actions;
  } catch (err) {
