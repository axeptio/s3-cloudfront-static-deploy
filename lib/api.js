  ...
  return new Promise((resolve, reject) => {
    s3.deleteObjects(params, (err, data) => {
      if (err) {
        ...
        success(onEvent, 'deleteFiles', 'the latest version of the files has received the deletion marker');
        resolve(data);
      }
    });
  });
...
async function rollbackFiles(clients, { bucket, filenames, distributionId }, onEvent) {
  try {
    ...
    success(onEvent, 'rollbackFiles', 'the latest version of the file received the deletion marker and was invalidate on cloudfront');
    return actions;
  } catch (err) {
