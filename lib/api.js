 ...
 */
function deleteFiles({ s3 }, { bucket, filenames }, onEvent) {
  const params = {
    Bucket: bucket,
    Delete: {
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        success(onEvent, 'rollbackFile', 'the latest version of the file received the deletion marker and was invalidated on cloudfront');
        resolve(actions);
      } catch (err) {
...
async function rollbackFiles(clients, { bucket, filenames, distributionId }, onEvent) {
  try {
    ...
    success(onEvent, 'rollbackFiles', 'the latest version of the files received the deletion marker and were invalidated on cloudfront');
    return actions;
  } catch (err) {
  ...
  try {
    ...
    if (!skipInvalidation) {
      actions.push(await invalidateFiles(
        clients, { distributionId, filenames: cloudfrontFilenames || filenames }, onEvent
      ));
    }
